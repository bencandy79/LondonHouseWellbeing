...
try:
    ...
    scaled_values = crime_scaler.fit_transform(df_crime_2017) 
    df_crime_2017.loc[:,:] = scaled_values
...
try:
    ...
    traffic_columns = ['Unnamed: 0','Unnamed: 1','Unnamed: 2', '1 Fatal.9','2 Serious.9','3 Slight.9']
    df_traffic_fatalities = pd.DataFrame(df_traffic_f, columns = traffic_columns)
    df_traffic_fatalities.rename(columns={'Unnamed: 0':'Ward_Code','Unnamed: 1':'Ward_Name','Unnamed: 2':'Borough', '1 Fatal.9':'Fatal','2 Serious.9':'Serious','3 Slight.9':'Slight'}, inplace=True)
    df_traffic_fatalities['score']=(df_traffic_fatalities['Fatal']*10)+(df_traffic_fatalities['Serious']*5)+(df_traffic_fatalities['Slight']*1)
except:
